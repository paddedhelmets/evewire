...
def industry_summary(request: HttpRequest, character_id: int = None) -> HttpResponse:
    ...
    # Build industry summary by pilot for account-wide view
    industry_by_pilot = None
    if is_account_wide:
        industry_by_pilot = []
        for char in all_characters:
            industry_by_pilot.append({
                'character': char,
                'manufacturing': {
                    'slots': char.manufacturing_slots,
                    'active': char.active_manufacturing_jobs,
                    'utilization': char.manufacturing_utilization,
                },
                'science': {
                    'slots': char.science_slots,
                    'active': char.active_research_jobs,
                    'utilization': char.science_utilization,
                },
                'reactions': {
                    'slots': char.reaction_slots,
                    'active': char.active_reaction_jobs,
                    'utilization': char.reaction_utilization,
                },
            })
    ...
    return render(request, 'core/industry_summary.html', {
        ...
        'industry_by_pilot': industry_by_pilot,
    })
...
def industry_jobs_list(request: HttpRequest, character_id: int = None) -> HttpResponse:
    ...
    activity_filter = request.GET.get('activity', '')  # manufacturing, research, etc.
    status_filter = request.GET.get('status', '')  # active, paused, completed
    ...
    if is_account_wide:
        ...
        if pilot_filter:
            ...
            jobs_qs = IndustryJob.objects.filter(character_id__in=pilot_ids)
        else:
            jobs_qs = IndustryJob.objects.filter(character__user=request.user)
    else:
    ...
    if status_filter:
        if status_filter == 'active':
            jobs_qs = jobs_qs.filter(status='active')
        elif status_filter == 'paused':
            jobs_qs = jobs_qs.filter(status='paused')
        elif status_filter == 'completed':
            jobs_qs = jobs_qs.filter(status__in=['ready', 'delivered'])
    ...
    # Define activity and status filter options
    activity_options = {
        '': 'All Activities',
        '1': 'Manufacturing',
        '3': 'Researching Technology',
        '4': 'Researching Material Efficiency',
        '5': 'Researching Time Efficiency',
        '7': 'Invention',
